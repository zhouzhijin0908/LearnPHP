...
*/
require_once ('DBShare.php');
$ezsql_mysql_str = array(1 =>
    'Require $dbuser and $dbpassword to connect to a database server', 2 =>
    'Error establishing mySQL database connection. Correct user/password? Correct hostname? Database server running?',
    3 => 'Require $dbname to select a database', 4 =>
    'mySQL database connection is not active', 5 =>
...
if (!function_exists('mysql_connect'))
    die('<b>Fatal Error:</b> DB requires mySQL Lib to be compiled and or linked in to the PHP engine');
if (!class_exists('DBShare'))
    die('<b>Fatal Error:</b> DB requires DBShare (DBShare.php) to be included/loaded before it can be used');

class DB extends DBShare
{
    ...
    */
    ...
    function connect($dbuser = '', $dbpassword = '', $dbhost = 'localhost')
    {
        ...
        if (!$dbuser) {
            $this->register_error($ezsql_mysql_str[1] . ' in ' . __file__ . ' on line ' .
                __line__);
        ...
        // Try to establish the server database handle
            ...
            if (!$this->dbh = @mysql_connect($dbhost, $dbuser, $dbpassword, true)) {
                $this->register_error($ezsql_mysql_str[2] . ' in ' . __file__ . ' on line ' .
                    __line__);
    ...
    */
    ...
    function select($dbname = '')
    {
        ...
        if (!$dbname) {
            $this->register_error($ezsql_mysql_str[3] . ' in ' . __file__ . ' on line ' .
                __line__);
        ...
        // Must have an active database connection
            ...
            if (!$this->dbh) {
                $this->register_error($ezsql_mysql_str[4] . ' in ' . __file__ . ' on line ' .
                    __line__);
